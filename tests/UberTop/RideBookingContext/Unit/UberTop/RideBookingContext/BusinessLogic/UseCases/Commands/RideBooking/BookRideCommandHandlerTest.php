...
use App\UberTop\RideBookingContext\Adapters\Secondary\TripScanning\TripScannerStub;
use App\UberTop\RideBookingContext\Adapters\Secondary\UuidGeneration\DeterministicUuidGenerator;
...
beforeEach(function () {
    ...
    $this->bookRide = new BookRideCommandHandler($this->rideRepository, $this->tripScanner,
        $this->dateProvider, $this->uuidGenerator);
    $this->dateProvider->setDateNow(new \DateTime('2022-03-04'));
    $this->riderId = UuidV4::fromString('b6b61e19-4e47-48db-a45a-dde8481b5a42');
    $this->rideId = UuidV4::fromString('c56ee94f-799a-46eb-97b2-2e5dece46339');
    $this->uuidGenerator->setNextUuids($this->rideId);
});
...
it('can book a basic ride with some distance', function (
    ...
    // ARRANGE
    $this->tripScanner->setDistances($distance);
    // ACT
    $this->bookRide->__invoke(new BookRideCommand($this->riderId, "8 avenue Foch Paris", $arrival, false));
    // ASSERT
    expectBookedRides($this->rideRepository, $this->rideId, $this->riderId, $arrival, $expectedPrice);
})->with([
...
it('should double the price because Christmas', function (
    ...
    // ASSERT
    expectBookedRides($this->rideRepository, $this->rideId, $this->riderId, "8 avenue Foch Paris", $expectedPrice);
})->with([
...
function expectBookedRides(RideRepositoryStub $rideRepository, $rideId, $riderId, $arrival, $expectedPrice): void
{
    expect($rideRepository->rides())->toEqual([
        new Ride(
            UuidV4::fromString($rideId),
            $riderId,
            "8 avenue Foch Paris",
            $arrival,
            $expectedPrice
